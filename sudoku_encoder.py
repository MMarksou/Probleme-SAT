...
#l'allocation en mémoire nous ont découragé.Voici la version en python.
...
def c3_c4_c5():
    ...
    for v in range(1, taille+1):
        for l in range(1, taille+1):
            ...
            sat.append([val_lit(l,c,v) for c in range(1,taille+1)])

        for c in range(1, taille+1):
            #si chaque colonne a une valeur v
            sat.append([val_lit(l,c,v) for l in range(1,taille+1)])
        ...
        for bl in range(0, bloc):
            for bc in range(0, bloc):
                ...
                for lb in range(1, taille+1) for bc in range(1, taille+1)])
    ...
    for l in range(1, taille+1):
        for c in range(1, taille+1):
            if(g[l-1][c-1] != '0'):
                ...
                sat.append([val_lit(l,c,int(g[l-1][c-1]))])
